...
public interface PptMapper {
    ...
    @Select("SELECT id,lesson_id,created_at,updated_at FROM ppt")
    @ResultMap(value = "ppt")
    List<Ppt> getAll();//查找所有记录，不提供pdf预览

    @Insert("INSERT INTO ppt (lesson_id,content,created_at,updated_at) VALUES (#{lessonId},#{content},#{createdTime},#{updatedTime})")
    boolean insert(Ppt ppt);

    @Delete("DELETE FROM ppt WHERE id=#{id}")
    boolean delete(int id);

    @Update("UPDATE ppt SET lesson_id=#{lessonId},content=#{content},updated_at=#{updatedTime} WHERE id=#{id}")
